...
export function MainLayout({ children }: MainLayoutProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      {/* Header */}
      <header className="bg-bg shadow-sm border-b border-border sticky top-0 z-50">
        <div className="container px-4">
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center space-x-8">
              <Link
                to="/"
                className="text-xl font-bold text-fg hover:text-primary transition-colors"
              >
                React Router App
              </Link>
                  ...
                  <Link
                    ...
                    className={`flex items-center space-x-2 px-3 py-2 rounded-md text-sm font-medium transition-all duration-200 ${
                      location.pathname === item.path
                        ? "bg-accent text-accent-fg border border-border"
                        : "text-muted-fg hover:text-fg hover:bg-accent border border-transparent hover:border-border"
                    }`}
                  >
                    <span>{item.icon}</span>
